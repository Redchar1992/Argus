...
import com.argus.auth.security.JwtAuthFilter;
...
public class SecurityConfig {
    ...
    public PasswordEncoder passwordEncoder() {
        // Real bcrypt with cost factor 12.
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/api/auth/internal/passkeys/**").permitAll()
                        .requestMatchers("/actuator/**").permitAll()
                        .anyRequest().authenticated())
                // Missing/invalid credentials -> 401 (not 403). 403 is reserved for an
                // authenticated caller who lacks the required role.
