...
export const DICT = {
  ...
  'auth.oidcSignIn': { en: 'Continue with OIDC', zh: '使用 OIDC 繼續' },
  'auth.localMock': { en: 'local mock provider', zh: '本機模擬提供者' },
  'auth.oidcFailed': {
    en: 'OIDC sign-in was rejected or expired. Try again.',
    zh: 'OIDC 登入遭拒或已逾時，請重試。',
  },
  'mfa.manage': { en: 'Manage MFA & recovery', zh: '管理 MFA 與復原' },
  'mfa.close': { en: 'Close MFA settings', zh: '關閉 MFA 設定' },
  'mfa.title': { en: 'Authenticator & recovery', zh: '驗證器與帳戶復原' },
  'mfa.intro': {
    en: 'Protect password and OIDC sign-in with RFC 6238 TOTP and one-time offline recovery codes.',
    zh: '使用 RFC 6238 TOTP 與一次性離線復原碼保護密碼及 OIDC 登入。',
  },
  'mfa.loading': { en: 'Loading security status…', zh: '載入安全狀態…' },
  'mfa.failed': { en: 'The MFA operation failed.', zh: 'MFA 操作失敗。' },
  'mfa.off': { en: 'MFA off', zh: 'MFA 未啟用' },
  'mfa.on': { en: 'MFA on', zh: 'MFA 已啟用' },
  'mfa.offDescription': {
    en: 'Your account currently needs only its primary sign-in factor.',
    zh: '您的帳戶目前只需要主要登入因素。',
  },
  'mfa.startSetup': { en: 'Set up authenticator', zh: '設定驗證器' },
  'mfa.setupTitle': { en: 'Enroll an authenticator', zh: '註冊驗證器' },
  'mfa.setupStep1': { en: 'Add a TOTP account in an authenticator app.', zh: '在驗證器應用程式中新增 TOTP 帳戶。' },
  'mfa.setupStep2': { en: 'Scan/open the URI or enter the secret below.', zh: '掃描或開啟 URI，或輸入下方密鑰。' },
  'mfa.setupStep3': { en: 'Enter a current six-digit code to confirm.', zh: '輸入目前的六位數驗證碼完成確認。' },
  'mfa.secret': { en: 'TOTP secret', zh: 'TOTP 密鑰' },
  'mfa.openAuthenticator': { en: 'Open provisioning URI', zh: '開啟設定 URI' },
  'mfa.localHelper': { en: 'Phone-free local demo helper', zh: '免手機本機示範工具' },
  'mfa.localHelperDescription': {
    en: 'Run this from the repository root. It computes a real TOTP; it does not bypass verification.',
    zh: '請在專案根目錄執行。它會計算真實 TOTP，不會繞過驗證。',
  },
  'mfa.enable': { en: 'Enable MFA', zh: '啟用 MFA' },
  'mfa.cancelSetup': { en: 'Cancel setup', zh: '取消設定' },
  'mfa.enrolled': { en: 'Enrolled', zh: '註冊時間' },
  'mfa.remaining': { en: 'Unused recovery codes:', zh: '未使用的復原碼：' },
  'mfa.regenerate': { en: 'Replace recovery codes', zh: '更換復原碼' },
  'mfa.disable': { en: 'Disable MFA', zh: '停用 MFA' },
  'mfa.recoveryTitle': { en: 'Save these recovery codes now', zh: '立即儲存這些復原碼' },
  'mfa.recoveryIntro': {
    en: 'Each code works once and plaintext is shown only in this response. Store them offline.',
    zh: '每組復原碼只能使用一次，明文只會在此回應顯示。請離線保存。',
  },
  'mfa.copyCodes': { en: 'Copy all codes', zh: '複製全部復原碼' },
  'mfa.copied': { en: 'Recovery codes copied.', zh: '已複製復原碼。' },
  'mfa.copyUnavailable': { en: 'Clipboard access is unavailable.', zh: '無法使用剪貼簿。' },
  'mfa.enabledNotice': { en: 'MFA enabled. Save the recovery codes before leaving.', zh: 'MFA 已啟用。離開前請儲存復原碼。' },
  'mfa.regeneratedNotice': { en: 'Old recovery codes were invalidated.', zh: '舊復原碼已失效。' },
  'mfa.disabledNotice': { en: 'TOTP MFA disabled.', zh: 'TOTP MFA 已停用。' },
