...
|---|---|---:|---:|---|
| 1 | The investigation API exposes a timeline, but the demo does not show a review gate or an explicit intervention state. | High | Medium | Add a deterministic `AUTO_APPROVED` / `HUMAN_REVIEW` state to the fixture contract and explain that it is a demo projection, not a production approval claim. |
...
The Pages implementation is a deterministic fixture, while the backend remains the
source of truth for live investigations.
